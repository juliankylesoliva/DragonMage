...
@export var yellow_magent_projectile_scene : PackedScene

@export var projectile_spawn_point : Node2D

@export var pre_fire_windup : float = 3

@export var post_fire_cooldown : float = 3

@export var lock_on_threshold : float = 0.5
...
@export var magic_windup_modifier : float = 0.5

@export var magic_cooldown_modifier : float = 0.5
...
func update_state_timer(delta : float):
	if (current_projectile_state == EnemyProjectileState.WINDUP or current_projectile_state == EnemyProjectileState.COOLDOWN):
		sprite.modulate = (Color.YELLOW if current_projectile_state == EnemyProjectileState.WINDUP else Color.GRAY)
		...
		if (current_projectile_state_timer <= 0):
			if (current_projectile_state == EnemyProjectileState.WINDUP):
				var proj_temp: Node = yellow_magent_projectile_scene.instantiate()
				body.add_sibling(proj_temp)
				(proj_temp as Node2D).global_position = projectile_spawn_point.global_position
				
				(proj_temp as EnemyProjectile).general_setup(self.global_position.direction_to(current_target_position))
...
func on_player_still_nearby():
	if (!is_defeated and visibility_notifier.is_on_screen()):
		if (player_detection.get_distance_to_player() > min_distance_from_player and player_detection.get_distance_to_player() <= max_distance_from_player):
			if (current_projectile_state == EnemyProjectileState.STANDBY):
				launch_projectile()
			elif (current_projectile_state == EnemyProjectileState.WINDUP):
				if (current_projectile_state_timer > lock_on_threshold):
					current_target_position = player_detection.get_player_position()
				else:
		...
		if (!is_defeated and current_projectile_state == EnemyProjectileState.WINDUP):
			reset_timer_and_state()
