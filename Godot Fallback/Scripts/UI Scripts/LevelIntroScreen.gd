...
@export var sprite_travel_distance : float = 832

@export var sprite_move_duration : float = 3.0

@export var blue_bg : Sprite2D

@export var orange_bg : Sprite2D

@export var mage_silhouette : AnimatedSprite2D

@export var dragon_silhouette : AnimatedSprite2D
...
@export var start_message_format : String = "[center]{cheer}, {name}!"
...
@export var start_cheers : Array[String]
...
func do_level_intro():
	slide_sprite_from_offset(blue_bg, Vector2.LEFT * screen_width, bg_combine_duration)
	slide_sprite_from_offset(orange_bg, Vector2.RIGHT * screen_width, bg_combine_duration)
	slide_to_offset(mage_silhouette, Vector2.RIGHT * sprite_travel_distance, sprite_move_duration)
	slide_to_offset(dragon_silhouette, Vector2.RIGHT * sprite_travel_distance, sprite_move_duration)
	...
	start_message_label.text = start_message_format.format({"cheer" : start_cheers.pick_random(), "name" : (dragon_name_image if FormSelectionHelper.is_dragon_selected() else mage_name_image)})
	...
	await get_tree().create_timer(0.5).timeout
	slide_sprite_to_offset(blue_bg, Vector2.LEFT * screen_width, bg_combine_duration)
	slide_sprite_to_offset(orange_bg, Vector2.RIGHT * screen_width, bg_combine_duration)
	intro_finished.emit()
...
func slide_sprite_from_offset(node : Sprite2D, offset : Vector2, time : float):
	var target_position : Vector2 = node.offset
	var lerp_value : float = 0
	node.set_visible(true)
	while (lerp_value < 1):
		lerp_value = move_toward(lerp_value, 1, get_physics_process_delta_time() / time)
		node.offset.x = lerp(target_position.x + offset.x, target_position.x, lerp_value)
		node.offset.y = lerp(target_position.y + offset.y, target_position.y, lerp_value)
		await get_tree().process_frame

func slide_sprite_to_offset(node : Sprite2D, offset : Vector2, time : float):
	var target_position : Vector2 = (node.offset + offset)
	var lerp_value : float = 0
	while (lerp_value < 1):
		lerp_value = move_toward(lerp_value, 1, get_physics_process_delta_time() / time)
		node.offset.x = lerp(target_position.x - offset.x, target_position.x, lerp_value)
