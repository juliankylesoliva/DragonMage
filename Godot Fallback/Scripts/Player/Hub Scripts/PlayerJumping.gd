...
func can_fast_fall():
	return (enable_fast_falling and !is_fast_falling and hub.buffers.is_fast_fall_buffer_active() and fast_falling_speed > max_fall_speed and !hub.char_body.is_on_floor() and !hub.collisions.is_in_ceiling_when_uncrouched() and hub.char_body.velocity.y >= fast_fall_threshold and hub.char_body.velocity.y < fast_falling_speed)
