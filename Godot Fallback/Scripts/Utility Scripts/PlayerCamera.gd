...
@export var standing_x_easing_rate : float = 1
...
func update_x_lookahead(delta : float):
	...
	current_x_lookahead = move_toward(current_x_lookahead, target_direction * max_lookahead, abs(((max_lookahead * target_direction) - current_x_lookahead) * standing_x_easing_rate if hub.get_input_vector().x == 0 else hub.char_body.velocity.x) * delta)
...
func move_locked_camera(delta : float):
	if (!enable_camera_lock or hub.is_deactivated):
		return
